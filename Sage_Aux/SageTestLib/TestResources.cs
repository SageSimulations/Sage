		...
		public void TestPersistentResourceBasics(){
			...
			for ( int i = 0; i < 7 ; i++ ) {
				if ( steamSystem.Reserve(requests[i],false) ){
					...
					Trace.WriteLine("Successfully reserved " + obtained + " pounds of steam - " + remaining + " remains.");
				} else {
					Trace.WriteLine("Failed to reserve steam for request["+i+"]");
				}
			}

			Trace.WriteLine("Unreserving steam from 2 requests");
			steamSystem.Unreserve(requests[2]);
			double available = steamSystem.Available;
			Trace.WriteLine("Successfully unreserved steam - " + available + " available.");
			...
			for ( int i = 5; i < 7 ; i++ ) {
				if ( steamSystem.Reserve(requests[i],false) ){
					...
					Trace.WriteLine("Failed to acquire steam for request["+i+"]");
				}
			}


			Trace.WriteLine("Unreserving all steam requests - ");
			for ( int i = 0; i < 7 ; i++ ) steamSystem.Unreserve(requests[i]);
			available = steamSystem.Available;
			Trace.WriteLine("Successfully unreserved steam - " + available + " available.");

			// AEL, bug "Reserve a resource over an existing one" submitted.
//			Trace.WriteLine("Trying to acquire all steam requests - ");
//			for ( int i = 0; i < 7 ; i++ ) {
//				if ( steamSystem.Acquire(requests[i],false) ){
//					double obtained = requests[i].QuantityObtained;
//					double remaining = requests[i].ResourceObtained.Available;
//					Trace.WriteLine("Successfully acquired " + obtained + " pounds of steam - " + remaining + " remains.");
//				} else {
//					Trace.WriteLine("Failed to acquire steam for request["+i+"]");
//				}
//			}
//
//			Trace.WriteLine("Releasing 2 steam requests ");
...
//			}
			...
			Trace.WriteLine("Releasing all steam requests - ");
		...
		public void TestConsumableResourceBasics(){
			...
			Model model = new Model("Resource Testing Model...");
			Trace.WriteLine("Test results of replenishable material inventories.");

			Hashtable materialInventory = new Hashtable();

			Trace.WriteLine("Setting up an inventory of 500 liters Water, capacity of 1000 liters.");
			...
			Trace.WriteLine("Setting up an inventory of 100 liters SodiumChloride, capacity of 250 liters.");
			...
			for ( int i = 0 ; i < tests.GetLength(0) ; i++ ) {
				...
				Trace.WriteLine("Before - " + itemName + " has " + item.Available + " liters, and a capacity of " + item.Capacity + ".");
				...
				Trace.Write((result?"Request honored.":"Request denied."));
				System.Diagnostics.Debug.Assert(result==expected,"This test is a failure");
				Trace.WriteLine(((result==expected)?" - this was expected.":" - THIS IS A TEST FAILURE!" )) ;
				System.Diagnostics.Debug.Assert(expected==result,testDescription); 
				Trace.WriteLine("After - " + itemName + " has " + item.Available + " liters, and a capacity of " + item.Capacity + ".");
				Trace.WriteLine("");
			}
		...
		public void TestMaterialConduits(){
			...
			Trace.WriteLine("Setting up an inventory of 500 liters Water, capacity of 1000 liters in materialInventory1.");
			materialInventory1.Add(WaterType,WaterItem);

			Trace.WriteLine("Setting up an inventory of 100 liters SodiumChloride, capacity of 250 liters in materialInventory1.");
			materialInventory1.Add(NaClType,NaClItem);

			Trace.WriteLine("Setting up an inventory of 750 liters Water, capacity of 1500 liters in materialInventory2.");
			materialInventory2.Add(WaterType,new MaterialResourceItem(model,WaterType,750,20,1500));

			Trace.WriteLine("Setting up an inventory of 400 liters SodiumChloride, capacity of 800 liters in materialInventory3.");
...
//			mcm.AddConduit(materialInventory3,NaClType);
			...
			for ( int i = 0 ; i < tests.GetLength(0) ; i++ ) {
				...
				System.Diagnostics.Debug.Assert(result==expected,"This test is a failure");
				Trace.WriteLine(((result==expected)?" - this was expected.":" - THIS IS A TEST FAILURE!" )) ;
				System.Diagnostics.Debug.Assert(result==expected,testDescription);
				Trace.WriteLine("After - " + itemName + " has " + item.Available + " liters, and a capacity of " + item.Capacity + ".");
				Trace.WriteLine("");
			}
		...
		public void TestEarmarking(){
			...
			Trace.WriteLine(rr.ResourceObtained==null?"Resource not obtained.":"Resource obtained.");
			rr.Release();
		...
		private void TryAcquire(SelfManagingResource rsc, IResourceRequest irr, bool expectSuccess){
			...
			if ( rsc.Acquire(irr,false) == expectSuccess ) {
				...
				System.Diagnostics.Debug.Assert(false,"Access Regulation","Sub-test failed : " + result);
			}
		}
